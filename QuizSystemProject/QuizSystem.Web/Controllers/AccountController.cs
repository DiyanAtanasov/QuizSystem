...
namespace QuizSystem.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            IdentityManager = new AuthenticationIdentityManager(new IdentityStore(new QuizContext()));
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = new QuizUser(model.UserName);
        ...
        [NonAction]
        protected async Task RegisterUserRole(QuizUser user, string role)
        {
            IRole roleUser = await IdentityManager.Roles.FindRoleByNameAsync(role);

            if (roleUser == null)
            {
                await IdentityManager.Roles.CreateRoleAsync(new Role { Name = role });
                roleUser = await IdentityManager.Roles.FindRoleByNameAsync(role);
            }

            await IdentityManager.Roles.AddUserToRoleAsync(user.Id, roleUser.Id);
        }
